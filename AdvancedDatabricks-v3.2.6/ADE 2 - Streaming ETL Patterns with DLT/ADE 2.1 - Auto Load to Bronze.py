...
display(dbutils.fs.ls("dbfs:/mnt/dbacademy-users/b-benltaief@ad-data-consulting.fr/advanced-data-engineering-with-databricks/streaming_etl/stream_source/daily"))
...
display(
    spark.table("bronze")
    .withColumn("keystr", col("key").cast("string"))
    .withColumn("valuejson", from_json(col("value").cast("string"), schema_json))
    .withColumn("timeformat", from_unixtime(col("valuejson.time")))
    .withColumn("timestampformat", from_unixtime(col("timestamp"), "HH:mm:ss"))
    .select("keystr", "valuejson.*", "timeformat", "topic", "partition", "offset", "timestampformat", "date", "week_part")
)
